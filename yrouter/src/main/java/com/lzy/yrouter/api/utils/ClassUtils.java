            ...
            DefaultPoolExecutor.getInstance().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        Logger.error("Scan map file in dex files made error.");
                    } finally {
                        if (null != dexfile) {
                            try {
                                dexfile.close();
...
//        通过是否存在sp中的multidex.version是不准确的，因为从低版本升级上来的用户，是包含这个sp配置的
        ...
        if (RouterCenter.debuggable()) { // Search instant run support only debuggable
    ...
    private static List<String> tryLoadInstantRunDexFile(ApplicationInfo applicationInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && null != applicationInfo.splitSourceDirs) {
            ...
            try {
                ...
                Logger.error("InstantRun support error, " + e.getMessage());
            }
        }
    ...
    /*private static void getClass(Context context, String packageName) {
        try {
